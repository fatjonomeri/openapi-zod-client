...
var getValidationMessage = function getValidationMessage(schema, key) {
  var _schema$xValidation;
  return (_schema$xValidation = schema["x-validation-messages"]) === null || _schema$xValidation === void 0 ? void 0 : _schema$xValidation[key];
};

// Helper to format message for Zod (escape quotes and handle special chars)
var formatMessageForZod = function formatMessageForZod(message) {
...
function getZodSchema(_ref) {
  ...
  var meta = _objectSpread2(_objectSpread2({}, inheritedMeta), {}, {
  ...
  if (schemaType && isPrimitiveType$1(schemaType)) {
    ...
    var extSchema = schema;
    var requiredMessage = meta !== null && meta !== void 0 && meta.isRequired ? getValidationMessage(extSchema, "required") : undefined;
    var baseType = tsPattern.match(schemaType)["with"]("integer", function () {
      return requiredMessage ? "z.number({ required_error: \"".concat(formatMessageForZod(requiredMessage), "\" })") : "z.number()";
    })["with"]("string", function () {
      return tsPattern.match(schema.format)["with"]("binary", function () {
        return "z.instanceof(File)";
      }).otherwise(function () {
        return requiredMessage ? "z.string({ required_error: \"".concat(formatMessageForZod(requiredMessage), "\" })") : "z.string()";
      });
    })["with"]("boolean", function () {
      return requiredMessage ? "z.boolean({ required_error: \"".concat(formatMessageForZod(requiredMessage), "\" })") : "z.boolean()";
    }).otherwise(function (type) {
      return requiredMessage ? "z.".concat(type, "({ required_error: \"").concat(formatMessageForZod(requiredMessage), "\" })") : "z.".concat(type, "()");
    });
    var chain = getZodChain({
      schema: schema,
      meta: meta,
      options: options
    });
    return code.assign("".concat(baseType).concat(chain));
  }
...
var getZodChain = function getZodChain(_ref6) {
  ...
  var backendValidations = [];
  // Backend-only validations that can't be checked on frontend
  var backendRules = ["exists", "unique", "same", "confirmed", "in"];
  if (extSchema["x-validation-messages"]) {
    for (var _i = 0, _Object$entries = Object.entries(extSchema["x-validation-messages"]); _i < _Object$entries.length; _i++) {
      var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
        rule = _Object$entries$_i[0],
        message = _Object$entries$_i[1];
      if (backendRules.includes(rule)) {
        backendValidations.push("".concat(message));
      }
    }
  }
  if (backendValidations.length > 0 && options !== null && options !== void 0 && options.withDescription) {
    var backendNote = "\n\n⚠️ Backend validations: " + backendValidations.join("; ");
    description = description ? description + backendNote : backendNote.trim();
  }
  if (typeof description === "string" && description !== "" && options !== null && options !== void 0 && options.withDescription) {
    if (["\n", "\r", "\r\n"].some(function (c) {
      return String.prototype.includes.call(description, c);
    })) {
      chains.push("describe(`".concat(description, "`)"));
    } else {
      chains.push("describe(\"".concat(description, "\")"));
    }
  }
...
var getZodChainableStringValidations = function getZodChainableStringValidations(schema) {
  ...
  var isBinaryFile = schema.format === "binary";
  if (!schema["enum"] && !isBinaryFile) {
    if (schema.minLength !== undefined) {
      var message = getValidationMessage(extSchema, "min");
      validations.push(message ? "min(".concat(schema.minLength, ", \"").concat(formatMessageForZod(message), "\")") : "min(".concat(schema.minLength, ")"));
    }
    if (schema.maxLength !== undefined) {
      var _message = getValidationMessage(extSchema, "max");
      validations.push(_message ? "max(".concat(schema.maxLength, ", \"").concat(formatMessageForZod(_message), "\")") : "max(".concat(schema.maxLength, ")"));
    }
  }
  if (schema.pattern) {
    var _message2 = getValidationMessage(extSchema, "regex");
    validations.push(_message2 ? "regex(".concat(formatPatternIfNeeded(schema.pattern), ", \"").concat(formatMessageForZod(_message2), "\")") : "regex(".concat(formatPatternIfNeeded(schema.pattern), ")"));
  }
  if (schema.format) {
    var formatType = schema.format;
    var _message3 = getValidationMessage(extSchema, formatType);
    var chain = tsPattern.match(formatType)["with"]("email", function () {
      return _message3 ? "email(\"".concat(formatMessageForZod(_message3), "\")") : "email()";
    })["with"]("hostname", function () {
      return _message3 ? "url(\"".concat(formatMessageForZod(_message3), "\")") : "url()";
    })["with"]("uri", function () {
      return _message3 ? "url(\"".concat(formatMessageForZod(_message3), "\")") : "url()";
    })["with"]("uuid", function () {
      return _message3 ? "uuid(\"".concat(formatMessageForZod(_message3), "\")") : "uuid()";
    })["with"]("date-time", function () {
      return _message3 ? "datetime({ offset: true, message: \"".concat(formatMessageForZod(_message3), "\" })") : "datetime({ offset: true })";
    }).otherwise(function () {
      return "";
    });
    if (chain) {
...
var getZodChainableNumberValidations = function getZodChainableNumberValidations(schema) {
  ...
  if (schema.type === "integer") {
    var message = getValidationMessage(extSchema, "int");
    validations.push(message ? "int(\"".concat(formatMessageForZod(message), "\")") : "int()");
  }
  if (schema.minimum !== undefined) {
    var _message4 = getValidationMessage(extSchema, "min") || getValidationMessage(extSchema, "gte");
    if (schema.exclusiveMinimum === true) {
      var gtMessage = getValidationMessage(extSchema, "gt");
      validations.push(gtMessage ? "gt(".concat(schema.minimum, ", \"").concat(formatMessageForZod(gtMessage), "\")") : "gt(".concat(schema.minimum, ")"));
    } else {
      validations.push(_message4 ? "gte(".concat(schema.minimum, ", \"").concat(formatMessageForZod(_message4), "\")") : "gte(".concat(schema.minimum, ")"));
    }
  } else if (typeof schema.exclusiveMinimum === "number") {
    var _message5 = getValidationMessage(extSchema, "gt");
    validations.push(_message5 ? "gt(".concat(schema.exclusiveMinimum, ", \"").concat(formatMessageForZod(_message5), "\")") : "gt(".concat(schema.exclusiveMinimum, ")"));
  }
  if (schema.maximum !== undefined) {
    var _message6 = getValidationMessage(extSchema, "max") || getValidationMessage(extSchema, "lte");
    if (schema.exclusiveMaximum === true) {
      var ltMessage = getValidationMessage(extSchema, "lt");
      validations.push(ltMessage ? "lt(".concat(schema.maximum, ", \"").concat(formatMessageForZod(ltMessage), "\")") : "lt(".concat(schema.maximum, ")"));
    } else {
      validations.push(_message6 ? "lte(".concat(schema.maximum, ", \"").concat(formatMessageForZod(_message6), "\")") : "lte(".concat(schema.maximum, ")"));
    }
  } else if (typeof schema.exclusiveMaximum === "number") {
    var _message7 = getValidationMessage(extSchema, "lt");
    validations.push(_message7 ? "lt(".concat(schema.exclusiveMaximum, ", \"").concat(formatMessageForZod(_message7), "\")") : "lt(".concat(schema.exclusiveMaximum, ")"));
  }
  if (schema.multipleOf) {
    var _message8 = getValidationMessage(extSchema, "multipleOf");
    validations.push(_message8 ? "multipleOf(".concat(schema.multipleOf, ", \"").concat(formatMessageForZod(_message8), "\")") : "multipleOf(".concat(schema.multipleOf, ")"));
  }
  return validations.join(".");
};
var getZodChainableArrayValidations = function getZodChainableArrayValidations(schema) {
  ...
  if (schema.minItems) {
    var message = getValidationMessage(extSchema, "min");
    validations.push(message ? "min(".concat(schema.minItems, ", \"").concat(formatMessageForZod(message), "\")") : "min(".concat(schema.minItems, ")"));
  }
  if (schema.maxItems) {
    var _message9 = getValidationMessage(extSchema, "max");
    validations.push(_message9 ? "max(".concat(schema.maxItems, ", \"").concat(formatMessageForZod(_message9), "\")") : "max(".concat(schema.maxItems, ")"));
  }
  return validations.join(".");
};
    ...
    return _regenerator().w(function (_context) {
      while (1) switch (_context.n) {
        ...
        case 9:
          ...
          break;
        case 10:
          return _context.a(2, outputByGroupName);
        case 11:
        ...
        case 12:
          return _context.a(2, prettyOutput);
      }
    }, _callee);
  }));
